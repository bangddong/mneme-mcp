...
- [x] **한국어 질의 정규화 (`korean.py`)**: 조사 어간 추출 + 음차 사전으로 FTS 질의를 확장.
      실측(위키 33건) — `비용은` 0→10건, `그라파나` 0→3건, `쿠버네티스 시크릿` 0→5건.
      **인덱스가 아니라 질의를 고쳤다** — 재색인 불필요(40~50분·LLM 필요), 되돌리기 쉬움.
## 3. TODO (우선순위 순)
...
- **한국어 검색은 인덱스가 아니라 질의를 고친다** (2026-08-08). `tokenize='unicode61'`은 한글 어절을 통째로 한 토큰으로 보므로 조사 하나에 0건이 된다(`비용` 12건 · `비용은` **0건**). 대안이던 `trigram`은 같은 코퍼스에서 **`비용`을 12→0건으로 죽였다**(3자 미만 질의 불가) — `비용`·`배포`·`인증` 같은 2글자 기술어가 흔해 손해가 크다. 음차(`그라파나`)는 **본문에 그 문자열이 아예 없어** 어떤 토크나이저로도 못 찾는다 → 사전만이 답. 질의 쪽 수정은 재색인이 필요 없고 되돌리기도 쉽다.
- **질의 확장은 순증(純增)이어야 한다**. `expand_token`은 항상 원본 토큰을 OR에 포함한다. 어간 추출이 틀려도 결과가 줄지 않는다 — 잘못된 어간의 최대 피해는 무관한 결과 몇 개(bm25가 대부분 흡수)이지만, 원본을 빼면 **되던 검색이 안 되게** 만든다. 후자가 훨씬 비싸다.
- 🔴 **FTS5는 괄호 그룹 사이의 암묵적 AND를 지원하지 않는다** (2026-08-08 실측). `(a OR b) (c OR d)` → `fts5: syntax error near "("`, `(a OR b) AND (c OR d)` → 정상. 첫 구현이 공백으로 이었다가 복합 질의에서 **조용히 0건**을 냈다. 낱말 질의는 전부 동작해 단어 테스트로는 안 잡혔고, `search_fts`의 구문 오류 폴백이 예외를 삼켜 "결과 없음"으로 보이게 만들었다 — **안전망이 버그를 가렸다.** 그래서 `test_확장식이_FTS5에서_구문오류를_내지_않는다`는 폴백 **바깥**에서 MATCH를 직접 태워 검사한다.
